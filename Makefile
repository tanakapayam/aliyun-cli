...
build: metas
	go build -ldflags "-s -w -X 'github.com/aliyun/aliyun-cli/cli.Version=${VERSION}'" -o out/aliyun main/main.go

install: build
	cp out/aliyun /usr/local/bin

build_mac:
	GOOS=darwin GOARCH=amd64 go build -ldflags "-s -w -X 'github.com/aliyun/aliyun-cli/cli.Version=${VERSION}'" -o out/aliyun main/main.go
...
build_linux:
	GOOS=linux GOARCH=amd64 go build -ldflags "-s -w -X 'github.com/aliyun/aliyun-cli/cli.Version=${VERSION}'" -o out/aliyun main/main.go
...
build_windows:
	GOOS=windows GOARCH=amd64 go build -ldflags "-s -w -X 'github.com/aliyun/aliyun-cli/cli.Version=${VERSION}'" -o aliyun.exe main/main.go
